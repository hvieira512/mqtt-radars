...
export const parseRectangle = (rectangle) =>
    rectangle.replace(/[{}]/g, "").split(";").map(p => p.trim().split(",").map(Number)).flat();
...
export const parseAreas = (data) => {
    ...
    return data.split("},")
        ...
        .map(area => {
            ...
            const type = vals[1];
            const coords = [];
            for (let i = 2; i < vals.length; i += 2) coords.push(vals[i], vals[i + 1]);
...
export const getAreaName = (data, key, type) => {
    ...
    if (data?.declare_area_name?.[key]) {
        ...
        label = parts.length > 1 ? parts.slice(1).join("_") : data.declare_area_name[key];
    }
    return label;
};
